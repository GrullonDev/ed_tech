...
| `ActivityEvent` | `circles/{circleId}/activityEvents` + `users/{uid}/activityEvents` | Lo escriben solo las Cloud Functions (triggers de Firestore), nunca el cliente. |

## Por qué el feed y los escudos viven en Cloud Functions, no en el cliente
...
`activityEvents`, `streakShieldGrants` ni `streakShieldUses` — solo el
Admin SDK (que usan las Cloud Functions) puede, porque ignora las reglas.
...
- **`streakShieldGrants` / `streakShieldUses` / `memberStats` /
  `activityEvents`**: de solo lectura para el cliente, `write: false` —
  únicamente las Cloud Functions escriben ahí.
