...
2. **Fase 1**: agregar `firebase_core`, `firebase_auth`, `cloud_firestore`
   (+ `cloud_functions` si se usa la ruta con Cloud Functions) al
   `pubspec.yaml`; correr `flutterfire configure` contra el proyecto
   `rachatribu`; implementar Auth anónimo + `users/{uid}`.
   `HomeLogic` sigue usando `LocalHabitRepository` para todo lo demás.
